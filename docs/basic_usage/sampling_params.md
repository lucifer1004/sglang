...
| return_routed_experts      | `bool = False`                                                               | Whether to return routed experts for MoE models. Requires `--enable-return-routed-experts` server flag. Normally returns base64-encoded int32 expert IDs as a flattened array with logical shape `[num_tokens, num_layers, top_k]`. For optimized paths such as WeLM KV-mirror prefill where some layer-token routes are not computed, returns a dict with encoded `values`, `valid_mask`, shape metadata, and `missing_value=-1`. |
| forced_decode_token_ids    | `Optional[Union[List[int], List[List[int]]]] = None`                         | Force the generated decode token IDs after each forward has run. Intended for WeLM v4 80A3 benchmarking on `/generate`; `max_new_tokens` must match the forced sequence length. |
