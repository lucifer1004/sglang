...
inline void build_ngram_with_tree(
    ...
    int topk,
    int i) {
  cudaStream_t stream = host::LaunchKernel::resolve_device(parent_list.device());
...
inline void build_ngram_with_target_verify(
    ...
    int draft_token_num,
    int buffer_size) {
  cudaStream_t stream = host::LaunchKernel::resolve_device(seq_lens.device());
...
inline void assign_ngram_input_ids_draft_extend_after_decode(
    ...
    int buffer_size,
    int update_buffer) {
  cudaStream_t stream = host::LaunchKernel::resolve_device(accept_length.device());
  int bs = 1;
  for (int d = 0; d < accept_length.dim(); ++d)
    bs *= accept_length.size(d);
  assign_ngram_input_ids_draft_extend_after_decode_kernel<<<bs, 32, 0, stream>>>(
      ...
      gram_n,
      buffer_size,
      static_cast<bool>(update_buffer));
}
