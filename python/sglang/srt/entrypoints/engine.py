    ...
    if not get_bool_env_var("SGLANG_SKIP_SGL_KERNEL_VERSION_CHECK"):
        if server_args.attention_backend == "flashinfer":
            try:
                ...
                logger.warning(
                    "Proceeding with older flashinfer_python because the local "
                    "environment cannot currently upgrade dependencies: %s",
                    exc,
                )
        if _is_cuda:
            try:
                ...
                logger.warning(
                    "Proceeding without an installed sglang-kernel package metadata "
                    "check because the local environment uses a pre-existing kernel "
                    "setup: %s",
                    exc,
                )
