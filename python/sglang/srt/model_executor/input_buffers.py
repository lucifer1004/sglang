...
class ForwardInputBuffers:
    ...
    def share_buffers(self):
        ...
        for f in fields(self):
            ...
            if not isinstance(buffer, (torch.Tensor, dict, list)) and not dataclasses.is_dataclass(buffer):
                continue
            ...
            if isinstance(buffer, dict):
                ...
                for idx, sub_buffer in enumerate(buffer):
                    assert isinstance(
                        sub_buffer, torch.Tensor
                    ), f"Field {name}[{idx}] is expected to be a torch.Tensor, but got {type(sub_buffer)}."
                    buffer[idx] = self._share_one_buffer(
                        f"{name}[{idx}]", sub_buffer
                    )
            else:
                assert isinstance(
                    buffer, torch.Tensor
