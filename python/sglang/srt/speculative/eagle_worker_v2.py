...
class EagleDraftWorker(BaseDraftWorker):
    ...
    def _get_welmv4_mtp_step_dp_counts(
        self,
        forward_batch: ForwardBatch,
        local_rows: int,
    ) -> Optional[List[int]]:
        ...
        if tp_group.world_size % num_dp_slots != 0:
            raise RuntimeError(
                "WeLM MTP DP step-count gather expected the TP group size to be "
