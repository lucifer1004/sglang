...
class ReasoningParser:
    ...
    DetectorMap: Dict[str, Type[BaseReasoningFormatDetector]] = {
        ...
        "welm-v4": WelmV4ReasoningDetector,
    }
    ...
    @classmethod
    def accepts_token_ids_for(cls, model_type: Optional[str]) -> bool:
        detector_class = cls.DetectorMap.get((model_type or "").lower())
    ...
    @property
    def remaining_token_ids(self) -> Optional[List[int]]:
        remaining = getattr(self.detector, "remaining_token_ids", None)
        return list(remaining or []) if remaining is not None else None
    ...
    def parse_non_stream(
        ...
        ``token_ids`` is forwarded to WeLM-v4 and ignored by text-based
        detectors.
        """
        ret = self.detector.detect_and_parse(full_text, *self._detector_args(token_ids))
    ...
    def parse_stream_chunk(
        ...
        ``token_ids`` is the output-id delta for this chunk; forwarded to
        WeLM-v4 and ignored by text-based detectors.
        """
        ret = self.detector.parse_streaming_increment(
            chunk_text, *self._detector_args(token_ids)
        )
        return ret.reasoning_text, ret.normal_text
