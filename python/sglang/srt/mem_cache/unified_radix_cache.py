...
class UnifiedLRUList:
    ...
    def _lock_ref(self, node: UnifiedTreeNode) -> int:
        cd = node.component_data[self.component_type]
        return cd.host_lock_ref if self._use_host_ptr else cd.lock_ref
    ...
    def get_prev_leaf_no_lock(self, node: UnifiedTreeNode, check_id: bool = True):
        ...
        pt = self._pt
        x = node.lru_prev[pt]
        while self._lock_ref(x) > 0 or len(x.children) > 0:
            x = x.lru_prev[pt]
        if x == self.head:
            return None
        return x
...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def check_prefetch_progress(self, req_id: str) -> bool:
        ...
        if operation.pool_storage_result is not None:
            extra_pool_hit_pages = operation.pool_storage_result.extra_pool_hit_pages
        logger.info(
            "HiCache prefetch success req=%s completed_local=%d completed_synced=%d matched=%d loaded=%d tail_release=%d occupied=%d extra_pool_hit_pages=%s",
            req_id,
            completed_tokens,
    ...
    def pending_storage_work_count(self) -> int:
        count = len(self.ongoing_write_through) + len(self.ongoing_load_back)
        count += len(self.ongoing_prefetch) + len(self.ongoing_backup)

        cc = self.cache_controller
        if cc is None:
            return count

        count += len(getattr(cc, "ack_write_queue", []))
        count += len(getattr(cc, "ack_load_queue", []))
        ...
        for queue_name in (
            ...
            queue = getattr(cc, queue_name, None)
            if queue is not None:
                count += queue.qsize()

        for queue in getattr(cc, "extra_host_mem_release_queues", {}).values():
            count += queue.qsize()
        return count

    def has_pending_storage_work(self) -> bool:
        return self.pending_storage_work_count() > 0

    def poll_storage_work(self) -> None:
        self.writing_check()
        self.loading_check()
        if self.enable_storage:
    ...
    def _flush_pending_storage_backups_before_reset(self) -> None:
        ...
        while time.monotonic() < deadline:
            self.drain_storage_control_queues()
            if not self.has_pending_storage_work():
                return
            time.sleep(0.05)

        logger.warning(
            "Timed out waiting for HiCache storage work to drain before reset: "
            "pending=%s ongoing_write=%s ongoing_load=%s ongoing_prefetch=%s "
            "ongoing_backup=%s",
            self.pending_storage_work_count(),
