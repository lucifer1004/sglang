...
_WELM_DUMP_ENABLED = (
    os.getenv("SGLANG_DUMP_ACTIVATIONS", "0").strip().lower() in _WELM_TRUE_ENV_VALUES
...
def _welm_previous_precision_rmsnorm(norm: RMSNorm, x: torch.Tensor) -> torch.Tensor:
    ...
    out = _sgl_rmsnorm_internal(x, norm.weight.data, norm.variance_epsilon, None, None)
    if needs_reshape:
...
def _pack_welm_kv_mirror_pp_tensor(
    ...
    if tp_size > 1:
        pad_len = 1
        while (flat.numel() + pad_len) % tp_size == 0:
            pad_len += 1
    proxy_tensors[key] = torch.cat([flat, flat.new_empty(pad_len)]) if pad_len else flat
    proxy_tensors[f"{key}.shape"] = tuple(tensor.shape)


def _unpack_welm_kv_mirror_pp_tensor(
...
def _pack_welm_kv_mirror_states(
    proxy_tensors: Dict[str, Any],
    kv_mirror_states: Optional[Dict[int, Tuple[torch.Tensor, torch.Tensor]]],
) -> None:
    if not kv_mirror_states:
        return
    for layer_idx, (mirror_k, mirror_v) in kv_mirror_states.items():
        _pack_welm_kv_mirror_pp_tensor(
            proxy_tensors, _welm_kv_mirror_pp_key(layer_idx, "k"), mirror_k
        )
        _pack_welm_kv_mirror_pp_tensor(
            proxy_tensors, _welm_kv_mirror_pp_key(layer_idx, "v"), mirror_v
        )


def _unpack_welm_kv_mirror_states(
    pp_proxy_tensors: PPProxyTensors,
) -> Dict[int, Tuple[torch.Tensor, torch.Tensor]]:
    kv_mirror_states = _empty_welm_kv_mirror_states()
    for key, tensor in pp_proxy_tensors.tensors.items():
        if not key.startswith(f"{WELM_KV_MIRROR_PP_KEY_PREFIX}."):
            continue
        if key.endswith(".shape"):
            continue
        _, layer_idx, suffix = key.rsplit(".", 2)
        tensor = _unpack_welm_kv_mirror_pp_tensor(pp_proxy_tensors, key, tensor)
        if suffix == "k":
            mirror_k, mirror_v = kv_mirror_states.get(int(layer_idx), (None, None))
            kv_mirror_states[int(layer_idx)] = (tensor, mirror_v)
        elif suffix == "v":
            mirror_k, mirror_v = kv_mirror_states.get(int(layer_idx), (None, None))
            kv_mirror_states[int(layer_idx)] = (mirror_k, tensor)
    return {
        layer_idx: (mirror_k, mirror_v)
        for layer_idx, (mirror_k, mirror_v) in kv_mirror_states.items()
        if mirror_k is not None and mirror_v is not None
    }
...
def _welm_update_contracted_dp_metadata(
    ...
    *,
    ...
    if contract_to_request_counts:
        ...
        force_extend_sum_len = num_dp_slots > 1 and (
...
def _get_welm_qkv_wna16_args(quant_config, layer, prefix):
    ...
    if (
        num_bits not in (4, 8)
        or str(qtype).lower().endswith("int") is False
        or not symmetric
    ):
        return None
...
class _WelmQkvDequantMethod:
    ...
    def process_weights_after_loading(self, layer: nn.Module) -> None:
        ...
        deq = (codes.reshape(out, in_features // gs, gs) * scale.unsqueeze(-1)).reshape(
            out, in_features
        )
...
class _WelmQkvMarlinMethod:
    ...
    def process_weights_after_loading(self, layer: nn.Module) -> None:
        ...
        if (
            ...
            and cached_w.shape == repacked.shape
            and cached_w.dtype == repacked.dtype
        ):
        ...
        if (
            cached_s is not None
            and cached_s.shape == scale_perm.shape
            and cached_s.dtype == scale_perm.dtype
        ):
    ...
    def restore_weights_before_loading(self, layer: nn.Module) -> None:
        ...
        if not hasattr(layer, "_original_shapes"):
            if (
                self.output_size_per_partition is None
                or self.input_size_per_partition is None
            ):
                return
...
class BaseWelmQkvProjection(nn.Module):
    ...
    def param_mapping(self, layer_idx: int) -> Dict[str, str]:
        ...
        for slot in self.shard_layout:
            m = re.match(
                r"^[kv]2(?:_(\d+))?$", slot
            )  # k2 / k2_N / v2 / v2_N -> mirror slot
            if m is None:
                tgt = layer_idx  # own layer: q / k1 / v1
            elif m.group(1) is not None:
                tgt = self.mirror_layer_indices[
                    int(m.group(1))
                ]  # multi-bank k2_N / v2_N
            else:
...
class Qwen2MoeAttention(nn.Module):
    ...
    def forward(
        ...
        if mtp_dump_attention_io:
            mtp_dump_prefix = f"model.mtp.0.decoder.{self.layer_idx}.layer.self_attn"
    ...
    def forward(
        ...
        if self.pp_group.is_first_rank:
            hidden_states = welm_embeddings(
                ...
                scale_seq_oe_embed_list=getattr(self, "scale_seq_oe_embed_list", None),
...
class WeLMV4MoeForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]], is_nextn=False):
        ...
        if is_nextn and is_welm_mtp_nextn:
            ...
            if not Envs.WELM_ALLOW_PARTIAL_MTP_LOAD.get() and (
                missing_projector_steps or missing_decoder_steps
            ):
                raise ValueError(
